...
public class BaselineSpringIntegrationTest extends AbstractBaselineSpringRestVerification {

    private @Autowired TestRestTemplate client;

    @Test
    public void helloViaClient() {
        ResponseEntity<String> response = this.client.getForEntity("/hello", String.class);
    ...
    public void meViaClient() {
        ResponseEntity<String> response = this.client.exchange("/me", HttpMethod.GET, new HttpEntity<>(this.getAuthenticatedHttpHeaders("user", "password")), String.class);
        assertEquals(HttpStatus.OK, response.getStatusCode());
