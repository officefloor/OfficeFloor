...
public class OfficeFloorNodeImpl implements OfficeFloorNode, ManagedObjectSourceVisitor, TeamVisitor {
     ...
     * {@link OfficeFloorManagedObjectSource} name.
     */
    private final Map<String, ManagedObjectSourceNode> inputManagedObjectSources = new HashMap<String, ManagedObjectSourceNode>();
    ...
    /**
     * Obtains all the {@link ManagedObjectSourceNode} instances.
     *
     * @return All the {@link ManagedObjectSourceNode} instances.
     */
    private Map<String, ManagedObjectSourceNode> getAllManagedObjectSources() {
        Map<String, ManagedObjectSourceNode> all = new HashMap<>();
        all.putAll(this.managedObjectSources);
        all.putAll(this.inputManagedObjectSources);
        return all;
    }
    ...
    public Node[] getChildNodes() {
        return NodeUtil.getChildNodes(this.teams, this.managedObjectSources, this.inputManagedObjectSources,
                this.inputManagedObjects, this.suppliers, this.managedObjects, this.offices);
    }
    ...
    public <O, M extends InputManagedObject> ExternalServiceInputFactory<O, M> addExternalServiceInputFactory(Class<O> objectType, String typeQualifier, Class<M> managedObjectType, OfficeNode managingOffice) {
        ...
        if (input != null) {
            ...
            // Create the input managed object source
            final ExternalServiceInputFactoryImpl<O, M> finalInput = input;
            OfficeFloorManagedObjectSource mos = NodeUtil.getInitialisedNode(inputObjectName, this.inputManagedObjectSources, this.context,
                    () -> this.context.createManagedObjectSourceNode(inputObjectName, this),
                    (managedObjectSourceNode) -> managedObjectSourceNode
                            .initialise(finalInput.getClass().getName(), finalInput));
    ...
    public boolean sourceOfficeFloorTree(AutoWirerVisitor autoWirerVisitor, CompileContext compileContext) {
        ...
        // Ensure any external service inputs are loaded
        isSourced = CompileUtil.source(this.inputManagedObjectSources,
                (managedObjectSource) -> managedObjectSource.getSectionManagedObjectSourceName(),
        ...
        // Obtain all the managed object sources
        Map<String, ManagedObjectSourceNode> allManagedObjectSources = this.getAllManagedObjectSources();
        ...
        if (this.isAutoWireObjects) {
            ...
            allManagedObjectSources.values().stream()
        ...
        boolean isAutoWireStartupOrdering = allManagedObjectSources.values().stream()
    ...
    public AutoWirer<ManagedObjectSourceNode> loadAutoWireManagedObjectSourceTargets(
        ...
        this.getAllManagedObjectSources().values().forEach((mos) -> {
    ...
    public OfficeFloorType loadOfficeFloorType(CompileContext compileContext) {
        ...
        OfficeFloorManagedObjectSourceType[] managedObjectSourceTypes = CompileUtil.loadTypes(this.getAllManagedObjectSources(),
    ...
    public OfficeFloor deployOfficeFloor(String officeFloorName, OfficeFloorBuilder builder,
        ...
        this.getAllManagedObjectSources().values().stream().sorted((a, b) -> CompileUtil
