...
public class ManagedObjectNodeImpl implements ManagedObjectNode {
    ...
    public void buildOfficeManagedObject(OfficeNode office, OfficeBuilder officeBuilder, OfficeBindings officeBindings,
        ...
        this.buildManagedObjectDependencies(managedObjectType, mapper, office, officeBindings, compileContext);
    ...
    public boolean buildFunctionManagedObject(String functionObjectName, ManagedFunctionBuilder<?, ?> functionBuilder,
                                              OfficeNode office, OfficeBindings officeBindings,
                                              CompileContext compileContext) {

        // Determine if function bound
        if (!ManagedObjectScope.FUNCTION.equals(this.state.managedObjectScope)) {
            return false; // not function bound
        }
        ...
        ManagedObjectType<?> managedObjectType = compileContext
                .getOrLoadManagedObjectType(this.state.managedObjectSourceNode);
        if (managedObjectType == null) {
            return false; // must have managed object type
        }

        // Add function managed object
        DependencyMappingBuilder mapper = functionBuilder.addManagedObject(functionObjectName, this.getBoundManagedObjectName());
    ...
    /**
     * Builds the dependencies of the {@link ManagedObject}.
     *
     * @param managedObjectType {@link ManagedObjectType}.
     * @param mapper            {@link DependencyMappingBuilder}.
     * @param office            {@link OfficeNode}.
     * @param officeBindings    {@link OfficeBindings}.
     * @param compileContext    {@link CompileContext}.
     */
    private void buildManagedObjectDependencies(ManagedObjectType<?> managedObjectType, DependencyMappingBuilder mapper,
