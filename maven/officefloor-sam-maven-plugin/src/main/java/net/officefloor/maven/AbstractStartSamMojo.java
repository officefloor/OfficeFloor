...
public abstract class AbstractStartSamMojo extends AbstractMojo {
	...
	/**
	 * Path of <code>template.yaml</code> on the class path.
	 */
	private static final String CLASSPATH_TEMPLATE_FILE_PATH = "/officefloor/sam/" + TEMPLATE_FILE_NAME;
	 ...
	 * Java runtime.
	 */
	@Parameter(defaultValue = "java17", property = "java.runtime")
	private String javaRuntime;

	/**
	 * Memory size.
	 */
	@Parameter(defaultValue = "512", property = "memory.size")
	...
	public void ensureTemplateYamlFileExists() throws MojoExecutionException {
		...
		try {
			InputStream input = this.getClass().getResourceAsStream(CLASSPATH_TEMPLATE_FILE_PATH);
			...
			template = template.replace("JAVA_RUNTIME", this.javaRuntime);
			template = template.replace("MEMORY_SIZE", this.memorySize);
