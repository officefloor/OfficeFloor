...
public class ChatEntryPoint implements EntryPoint {
		...
		messageText.addKeyDownHandler(new KeyDownHandler() {
			...
			public void onKeyDown(KeyDownEvent event) {
				if (KeyCodes.KEY_ENTER == event.getNativeKeyCode()) {
					ChatEntryPoint.this.sendChatMessage(messageText, publisher);
				}
			}
		});
	...
	private void sendChatMessage(final TextBox messageText,
		...
		String user = ChatEntryPoint.this.userName;
		if ((user == null) || (user.trim().length() == 0)) {
			...
			nameText.addKeyDownHandler(new KeyDownHandler() {
				...
				public void onKeyDown(KeyDownEvent event) {
					if (KeyCodes.KEY_ENTER == event.getNativeKeyCode()) {
						ChatEntryPoint.this.enterChatName(nameText, userDialog,
								messageText, publisher);
					}
				}
			});
	...
	private void enterChatName(TextBox nameText, DialogBox userDialog,
		...
		// Name provided
		ChatEntryPoint.this.userName = name;
		userDialog.hide();
