...
public class ClasspathHttpFileSenderWorkSourceTest extends OfficeFrameTestCase {
	...
	private final TaskContext<HttpFileFactoryTask<None>, Indexed, None> taskContext = this
	...
	public void testType() {
		...
		// Provide task type
		TaskTypeBuilder<Indexed, None> taskBuilder = workBuilder.addTaskType(
				"SendFile", task, Indexed.class, None.class);
		...
		WorkLoaderUtil
				.validateWorkType(
						...
						"index.html",
						ClasspathHttpFileSenderWorkSource.PROPERTY_FILE_NOT_FOUND_CONTENT_PATH,
						this.getFileNotFoundContentPath());
	}
	...
	public void testSendHttpFile() throws Throwable {
		...
		Task<HttpFileFactoryTask<None>, Indexed, None> task = this
	...
	public void testSendFileNotFoundContent() throws Throwable {
		...
		assertNotNull("Ensure have HTTP file returned", result);
		HttpFile httpFile = (HttpFile) result;
		assertFalse("HTTP file should be missing", httpFile.isExist());
	 ...
	 */
	@SuppressWarnings("unchecked")
	private final Task<HttpFileFactoryTask<None>, Indexed, None> loadWorkAndObtainTask() {
	...
	/**
	 * Obtains the location of the file not found content.
	 * 
	 * @return Path to the file containing the file not found content.
	 */
	private String getFileNotFoundContentPath() {
		// Obtain location of file not found content
		String fileNotFoundContentPath = this.getFileLocation(this.getClass(),
				"FileNotFoundContent.html");

		// Return the location
		return fileNotFoundContentPath;
	}
