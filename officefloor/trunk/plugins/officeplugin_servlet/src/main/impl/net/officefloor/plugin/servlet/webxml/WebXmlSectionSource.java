...
public class WebXmlSectionSource extends AbstractSectionSource {
	 ...
	 */
	public static void validateWebXmlConfiguration(InputStream webXmlContents, SourceContext context)
			throws InvalidServletConfigurationException {
		...
		try {
			...
			throw new InvalidServletConfigurationException(
					"Invalid web.xml configuration [" + ex.getClass().getSimpleName() + "]: " + ex.getMessage());
		}
	...
	public void sourceSection(SectionDesigner designer, SectionSourceContext context) throws Exception {
		...
		WebAppModel webApp = new WebXmlLoader().loadConfiguration(webXmlConfiguration, context);
		...
		SectionOutput servletExceptionOutput = designer.addSectionOutput(ServletException.class.getSimpleName(),
				ServletException.class.getName(), true);
		SectionOutput ioExceptionOutput = designer.addSectionOutput(IOException.class.getSimpleName(),
		...
		SectionObject httpConnectionMo = designer.addSectionObject("HTTP_CONNECTION",
				ServerHttpConnection.class.getName());
		SectionObject requestAttributesMo = designer.addSectionObject("REQUEST_ATTRIBUTES",
		...
		SectionManagedObjectSource officeServletContextMos = designer.addSectionManagedObjectSource(
				"OfficeServletContext", OfficeServletContextManagedObjectSource.class.getName());
		officeServletContextMos.addProperty(OfficeServletContextManagedObjectSource.PROPERTY_SERVLET_CONTEXT_NAME,
				servletContextName);
		for (ContextParamModel contextParam : webApp.getContextParams()) {
			officeServletContextMos.addProperty(
					OfficeServletContextManagedObjectSource.PROPERTY_PREFIX_INIT_PARAMETER + contextParam.getName(),
					contextParam.getValue());
		}
		SectionManagedObject officeServletContextMo = officeServletContextMos
				.addSectionManagedObject("OfficeServletContext", ManagedObjectScope.PROCESS);
		...
		for (MimeMappingModel mimeMapping : webApp.getMimeMappings()) {
			officeServletContextMos
					.addProperty(OfficeServletContextManagedObjectSource.PROPERTY_PREFIX_FILE_EXTENSION_TO_MIME_TYPE
		...
		for (FilterModel filter : webApp.getFilters()) {
			...
			officeServletContextMos.addProperty(
					OfficeServletContextManagedObjectSource.PROPERTY_FILTER_INSTANCE_NAME_PREFIX + filterName,
					filterClass);
			for (InitParamModel initParam : filter.getInitParams()) {
				officeServletContextMos
						.addProperty(OfficeServletContextManagedObjectSource.PROPERTY_FILTER_INSTANCE_INIT_PREFIX
								+ filterName + "." + initParam.getName(), initParam.getValue());
			}
		}
		...
		for (FilterMappingModel filterMapping : webApp.getFilterMappings()) {
			...
			for (String urlPattern : filterMapping.getUrlPatterns()) {
				...
				officeServletContextMos.addProperty(
						OfficeServletContextManagedObjectSource.PROPERTY_FILTER_MAPPING_INDEX_PREFIX + index,
						filterName);
				officeServletContextMos.addProperty(
						OfficeServletContextManagedObjectSource.PROPERTY_FILTER_MAPPING_URL_PREFIX + index, urlPattern);
				if (dispatchers != null) {
					officeServletContextMos.addProperty(
							OfficeServletContextManagedObjectSource.PROPERTY_FILTER_MAPPING_TYPE_PREFIX + index,
							dispatchers);
				}
			}
			...
			for (String servletName : filterMapping.getServletNames()) {
				...
				officeServletContextMos.addProperty(
						...
						filterName);
				officeServletContextMos.addProperty(
						OfficeServletContextManagedObjectSource.PROPERTY_FILTER_MAPPING_SERVLET_PREFIX + index,
						servletName);
				if (dispatchers != null) {
					officeServletContextMos.addProperty(
							OfficeServletContextManagedObjectSource.PROPERTY_FILTER_MAPPING_TYPE_PREFIX + index,
		...
		SectionTask routeTask = routeWork.addSectionTask("route", ServletRouteWorkSource.TASK_ROUTE);
		designer.link(serviceInput, routeTask);
		designer.link(routeTask.getTaskFlow(FlowKeys.UNHANDLED.name()), unhandledOutput,
				FlowInstigationStrategyEnum.SEQUENTIAL);
		designer.link(
				routeTask.getTaskObject(
						net.officefloor.plugin.servlet.route.ServletRouteTask.DependencyKeys.HTTP_CONNECTION.name()),
				httpConnectionMo);
		designer.link(routeTask.getTaskObject(
				net.officefloor.plugin.servlet.route.ServletRouteTask.DependencyKeys.OFFICE_SERVLET_CONTEXT.name()),
				officeServletContextMo);
		...
		for (ServletModel servletModel : servletModels) {
			...
			servlet.addProperty(HttpServletWorkSource.PROPERTY_SERVLET_NAME, servletName);
			servlet.addProperty(HttpServletWorkSource.PROPERTY_HTTP_SERVLET_CLASS_NAME, servletClass);
			servlet.addProperty(HttpServletWorkSource.PROPERTY_SERVLET_MAPPINGS, servletUrlPatterns);

			// Configure init-param values for servlet
			for (InitParamModel initParam : servletModel.getInitParams()) {
				servlet.addProperty(HttpServletWorkSource.PROPERTY_PREFIX_INIT_PARAMETER + initParam.getName(),
						initParam.getValue());
			}

			// Configure the servlet service task
			SectionTask servletTask = servlet.addSectionTask("service-by-" + servletName, HttpServletTask.TASK_NAME);
			servletTask.getTaskObject(DependencyKeys.SERVICER_MAPPING.name()).flagAsParameter();

			// Configure servlet dependencies
			designer.link(servletTask.getTaskObject(DependencyKeys.OFFICE_SERVLET_CONTEXT.name()),
					officeServletContextMo);
			designer.link(servletTask.getTaskObject(DependencyKeys.HTTP_CONNECTION.name()), httpConnectionMo);
			designer.link(servletTask.getTaskObject(DependencyKeys.REQUEST_ATTRIBUTES.name()), requestAttributesMo);
			designer.link(servletTask.getTaskObject(DependencyKeys.HTTP_SESSION.name()), httpSessionMo);
			designer.link(servletTask.getTaskObject(DependencyKeys.HTTP_SECURITY.name()), httpSecurityMo);

			// Configure the escalations
			// TODO consider web.xml error handling for this functionality
			designer.link(servletTask.getTaskEscalation(ServletException.class.getName()), servletExceptionOutput,
					FlowInstigationStrategyEnum.SEQUENTIAL);
			designer.link(servletTask.getTaskEscalation(IOException.class.getName()), ioExceptionOutput,
					FlowInstigationStrategyEnum.SEQUENTIAL);
		}
	}
