...
public class FilterInstance {
	...
	public static FilterInstance[] loadFilterInstances(PropertyList properties) {
		...
		for (String propertyName : properties.getPropertyNames()) {
			if (propertyName
					.startsWith(FilterChainFactoryManagedObjectSource.PROPERTY_FILTER_INSTANCE_NAME_PREFIX)) {

				// Obtain the filter instance name
				String name = propertyName
						.substring(FilterChainFactoryManagedObjectSource.PROPERTY_FILTER_INSTANCE_NAME_PREFIX
								.length());
	...
	public void outputProperties(PropertyList properties) {
		...
		String propertyName = FilterChainFactoryManagedObjectSource.PROPERTY_FILTER_INSTANCE_NAME_PREFIX
				+ this.name;
		properties.addProperty(propertyName).setValue(this.className);

		// Write out the filter init parameters
		String initParameterPrefix = FilterChainFactoryManagedObjectSource.PROPERTY_FILTER_INSTANCE_INIT_PREFIX
				+ this.name + ".";
