...
public class GovernanceSourceWizard extends Wizard implements GovernanceSourceInstanceContext {
	...
	public static GovernanceInstance getGovernanceInstance(AbstractOfficeFloorEditPart<?, ?, ?> editPart,
		...
		GovernanceSourceWizard wizard = new GovernanceSourceWizard(project, governanceInstance);
	...
	public static Map<String, GovernanceSourceInstance> createGovernanceSourceInstanceMap(ClassLoader classLoader,
			IProject project, GovernanceSourceInstanceContext context) {
		...
		try {
			...
			IType[] types = JavaUtil.getSubTypes(project, GovernanceSource.class.getName());
			for (IType type : types) {
				...
				governanceSourceInstances.put(className,
						new GovernanceSourceInstance(className, null, classLoader, project, context));
			}
		} catch (Throwable ex) {
		...
		for (GovernanceSourceExtension<?, ?, ?> governanceSourceExtension : ExtensionUtil
			...
			try {
				Class<?> governanceSourceClass = governanceSourceExtension.getGovernanceSourceClass();
				String governanceSourceClassName = governanceSourceClass.getName();
				governanceSourceInstances.put(governanceSourceClassName, new GovernanceSourceInstance(
						governanceSourceClassName, governanceSourceExtension, classLoader, project, context));
			} catch (Throwable ex) {
				LogUtil.logError(
						"Failed to create source instance for " + governanceSourceExtension.getClass().getName(), ex);
			}
		}
	 ...
	 */
	public GovernanceSourceWizard(IProject project, GovernanceInstance governanceInstance) {
		...
		GovernanceSourceInstance[] governanceSourceInstanceListing = governanceSourceInstanceMap.values()
				.toArray(new GovernanceSourceInstance[0]);
		Arrays.sort(governanceSourceInstanceListing, new Comparator<GovernanceSourceInstance>() {
			@Override
			public int compare(GovernanceSourceInstance a, GovernanceSourceInstance b) {
				return a.getGovernanceSourceClassName().compareTo(b.getGovernanceSourceClassName());
			}
		});

		// Create the pages
		this.listingPage = new GovernanceSourceListingWizardPage(governanceSourceInstanceListing, governanceInstance);
		for (GovernanceSourceInstance governanceSourceInstance : governanceSourceInstanceListing) {
			this.propertiesPages.put(governanceSourceInstance,
					new GovernanceSourcePropertiesWizardPage(this, governanceSourceInstance));
		}
		...
		if (governanceInstance != null) {
			...
			String governanceSourceClassName = governanceInstance.getGovernanceSourceClassName();
			for (GovernanceSourceInstance governanceSourceInstance : governanceSourceInstanceListing) {
				if (governanceSourceClassName.equals(governanceSourceInstance.getGovernanceSourceClassName())) {
					governanceSourceInstance.loadGovernanceInstance(governanceInstance);
				}
			}
		}
	}
	...
	public boolean performFinish() {
		...
		this.governanceInstance = new GovernanceInstance(governanceName, governanceSourceClassName, propertyList,
				governanceType);
