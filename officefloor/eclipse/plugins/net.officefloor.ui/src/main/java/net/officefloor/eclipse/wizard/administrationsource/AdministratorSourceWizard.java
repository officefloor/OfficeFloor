...
public class AdministratorSourceWizard extends Wizard implements
	...
	public static Map<String, AdministrationSourceInstance> createAdministratorSourceInstanceMap(
			ClassLoader classLoader, IProject project,
			AdministrationSourceInstanceContext context) {

		// Obtain the administrator source instances (unique set)
		Map<String, AdministrationSourceInstance> administratorSourceInstances = new HashMap<String, AdministrationSourceInstance>();

		// Obtain from project class path
		try {
			// Obtain the types on the class path
			IType[] types = JavaUtil.getSubTypes(project,
					AdministrationSource.class.getName());
			for (IType type : types) {
				...
				administratorSourceInstances.put(className,
						new AdministrationSourceInstance(className, null,
		...
		for (AdministrationSourceExtension administratorSourceExtension : ExtensionUtil
			...
			try {
				...
				administratorSourceInstances.put(administratorSourceClassName,
						new AdministrationSourceInstance(
	 ...
	 * {@link AdministrationSourceInstance}.
	 */
	private final Map<AdministrationSourceInstance, AdministratorSourcePropertiesWizardPage> propertiesPages = new HashMap<AdministrationSourceInstance, AdministratorSourcePropertiesWizardPage>();

	/**
	 * Selected {@link AdministrationSourceInstance}.
	 */
	private AdministrationSourceInstance selectedAdministratorSourceInstance = null;
	...
	public AdministratorSourceWizard(IProject project,
		...
		Map<String, AdministrationSourceInstance> administratorSourceInstanceMap = createAdministratorSourceInstanceMap(
				classLoader, project, this);

		// Obtain the listing of administrator source instances (in order)
		AdministrationSourceInstance[] administratorSourceInstanceListing = administratorSourceInstanceMap
				.values().toArray(new AdministrationSourceInstance[0]);
		Arrays.sort(administratorSourceInstanceListing,
				new Comparator<AdministrationSourceInstance>() {
					@Override
					public int compare(AdministrationSourceInstance a,
							AdministrationSourceInstance b) {
						return a.getAdministrationSourceClassName().compareTo(
								b.getAdministrationSourceClassName());
					}
				});
		...
		for (AdministrationSourceInstance administratorSourceInstance : administratorSourceInstanceListing) {
	...
	public boolean performFinish() {
		...
		PropertyList propertyList = this.selectedAdministratorSourceInstance
				.getPropertyList();
		AdministrationScope administratorScope = this.selectedAdministratorSourceInstance
