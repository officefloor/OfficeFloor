 ...
 */
public abstract class AbstractIdeEclipseEditor<R extends Model, RE extends Enum<RE>, O> extends AbstractFXEditor {
	...
	/**
	 * Enables running the {@link AbstractIdeEclipseEditor} outside of the
	 * {@link IWorkbench}.
	 * 
	...
	/**
	 * Convenience method to launch the {@link AbstractIdeEclipseEditor} implementation
	...
	public static void launch(String configuration) {
		...
		FOUND_LAUNCHER: for (StackTraceElement se : cause) {
			...
			if ((isFoundThisMethod) && (!className.equals(AbstractIdeEclipseEditor.class.getName()))) {
				launchClassName = className;
				break FOUND_LAUNCHER;
			} else if (AbstractIdeEclipseEditor.class.getName().equals(className) && "launch".equals(methodName)) {
		...
		if (launchClassName == null) {
			throw new RuntimeException("Unable to determine " + AbstractIdeEclipseEditor.class.getName() + " implementation");
		}
		...
		launchOutsideWorkbench(() -> {
			...
			AbstractIdeEclipseEditor editor = (AbstractIdeEclipseEditor) instance;
	 ...
	 * {@link AbstractIdeEclipseEditor}. Need to be unregistered on close of this
	 * {@link AbstractIdeEclipseEditor}.
	 */
	 ...
	 */
	private AbstractIdeEclipseEditor(Injector injector) {
		super(injector);
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	protected AbstractIdeEclipseEditor(AdaptedEditorModule module, Class<R> rootModelType, Function<R, O> createOperations) {
		...
		this.adaptedBuilder = (context) -> {
			...
			this.configurableContext = new ConfigurableContext<R, O>() {
				...
				public AdaptedRootBuilder<R, O> getRootBuilder() {
					return AbstractIdeEclipseEditor.this.rootBuilder;
				}

				@Override
				public OfficeFloorOsgiBridge getOsgiBridge() throws Exception {
					return AbstractIdeEclipseEditor.this.getOsgiBridge();
				}

				@Override
				public Shell getParentShell() {
					return AbstractIdeEclipseEditor.this.parentShell;
				}

				@Override
				public O getOperations() {
					return AbstractIdeEclipseEditor.this.operations;
				}

				@Override
				public ChangeExecutor getChangeExecutor() {
					return AbstractIdeEclipseEditor.this.rootBuilder.getChangeExecutor();
				}

				@Override
				public String getPreference(String preferenceId) {
					IPreferenceStore preferences = AbstractIdeEclipseEditor.this.preferenceStore;
				...
				public void addPreferenceListener(String preferenceId, PreferenceListener preferenceListener) {
					...
					if (preferences != null) {
						...
						AbstractIdeEclipseEditor.this.preferenceChangeListeners.add(changeListener);
					}
				}
			};
	 ...
	 *                         root {@link Model}.
	 */
	public AbstractIdeEclipseEditor(Class<R> rootModelType, Function<R, O> createOperations) {
