...
public class InputManagedObjectStressTest extends
	...
	private void doTest(Team team) throws Exception {
		...
		for (int i = 0; i < INSTANCE_COUNT; i++) {
			ManagedObjectBuilder<FlowKey> moBuilder = officeFloorBuilder
					.addManagedObject("INPUT_" + i,
							InputManagedObjectSource.class);
			moBuilder.addProperty("instance.index", String.valueOf(i));
			ManagingOfficeBuilder<FlowKey> managingBuilder = moBuilder
					.setManagingOffice(officeName);
			managingBuilder
					.linkProcess(FlowKey.TRIGGER_PROCESS, "WORK", "task");
			managingBuilder.setInputManagedObjectName("INPUT_MO")
					.mapDependency(DependencyKey.DEPENDENCY, "DEPENDENCY");
		}

		// Specify last input as bound
		officeBuilder.setBoundInputManagedObject("INPUT_MO", "INPUT_"
				+ (INSTANCE_COUNT - 1));

		// Run the asynchronous operations
		this.invokeWork("RUN_TEST", null, MAX_RUN_TIME);

		// Ensure correct number of invocations
		assertEquals("Incorrect number of invocations", INVOKE_PROCESS_COUNT,
				work.invocationCount.get());
	}
	...
	public class InputWork {
		...
		public void task(InputManagedObject mo) {
			...
			assertNotSame(
					"Should be different managed object source for input",
					this.previousInputManagedObjectSource, mo.source);
			this.previousInputManagedObjectSource = mo.source;

			// Ensure correct dependency
			assertEquals("Incorrect dependency", this.expectedDependency,
					mo.dependency);
	 ...
	 */
	public static class InputManagedObject implements
			CoordinatingManagedObject<DependencyKey> {
		 ...
		 */
		public InputManagedObject(InputManagedObjectSource source,
				int invocationCount) {
			this.source = source;
		...
		@Override
		public void loadObjects(ObjectRegistry<DependencyKey> registry)
				throws Throwable {
			// Obtain the dependency
			this.dependency = (String) registry
					.getObject(DependencyKey.DEPENDENCY);
		}
	 ...
	 */
	@TestSource
	public static class InputManagedObjectSource extends
			AbstractManagedObjectSource<DependencyKey, FlowKey> {
		...
		protected void loadMetaData(
				MetaDataContext<DependencyKey, FlowKey> context)
				throws Exception {
			ManagedObjectSourceContext<FlowKey> mosContext = context
					.getManagedObjectSourceContext();

			// Register this instance
			this.instanceIndex = Integer.parseInt(mosContext
					.getProperty("instance.index"));
			synchronized (instances) {
		...
		@Override
		public void start(ManagedObjectExecuteContext<FlowKey> context)
				throws Exception {
			synchronized (instances) {
