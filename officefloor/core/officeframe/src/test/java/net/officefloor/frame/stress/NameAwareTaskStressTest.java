...
public class NameAwareTaskStressTest extends AbstractOfficeConstructTestCase {
	...
	public void doTest(Team team) throws Exception {
		...
		this.constructManagedObject(BOUND_NAME, NameAwareManagedObjectSource.class, officeName);
		...
		// Run the repeats
		this.invokeFunction("nameAware", new Integer(1), MAX_RUN_TIME);
	...
	public class NameAwareManagedObjectTask {
		 ...
		 *            Expected bound name.
		 */
		public NameAwareManagedObjectTask(int maxSequentialCalls, String expectedBoundName) {
		 ...
		 */
		public void nameAware(Integer callCount, ReflectiveFlow flow, StressNameAwareManagedObject nameAware) {

			// Indicate the number of calls made
			this.sequentialCallCount.incrementAndGet();

			// Ensure the bound name is correct
			assertEquals("Incorrect bound name", this.expectedBoundName, nameAware.getBoundName());

			// Output progress
			if ((callCount.intValue() % 1000000) == 0) {
				NameAwareTaskStressTest.this.printMessage("Name aware task calls=" + callCount.intValue());
			}
	 ...
	 */
	@TestSource
	public static class NameAwareManagedObjectSource extends AbstractManagedObjectSource<None, None> {
