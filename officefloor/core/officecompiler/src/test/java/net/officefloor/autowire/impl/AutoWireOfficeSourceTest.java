...
public class AutoWireOfficeSourceTest extends OfficeFrameTestCase {
	 ...
	 */
	private final OfficeArchitect architect = this.createMock(OfficeArchitect.class);

	/**
	 * Mock {@link OfficeSourceContext}.
	 */
	private final OfficeSourceContext context = this.createMock(OfficeSourceContext.class);
	...
	public void testOverrideSection() throws Exception {
		...
		final OfficeSectionType officeSectionType = this.createMock(OfficeSectionType.class);
		final OfficeSection officeSection = this.createMock(OfficeSection.class);

		// Factory to override section
		final AutoWireSection overridden = this.createMock(AutoWireSection.class);
		...
		this.recordReturn(overridden, overridden.getSectionSourceClassName(), SectionSource.class.getName());
		this.recordReturn(overridden, overridden.getSectionLocation(), SECTION_LOCATION);
		this.recordReturn(this.architect,
				this.architect.addOfficeSection(SECTION, SectionSource.class.getName(), SECTION_LOCATION),
				officeSection);
		this.recordReturn(overridden, overridden.getProperties(), properties);
		properties.configureProperties(officeSection);
		this.recordReturn(this.context, this.context.loadOfficeSectionType(SECTION, SectionSource.class.getName(),
				SECTION_LOCATION, properties), officeSectionType);
		this.recordReturn(officeSectionType, officeSectionType.getOfficeSectionObjectTypes(),
		...
		this.recordReturn(officeSectionType, officeSectionType.getOfficeTaskTypes(), new OfficeTaskType[0]);
	...
	public void testTransformSection() throws Exception {
		...
		// Transformers
		final AutoWireSectionTransformer transformerOne = this.createMock(AutoWireSectionTransformer.class);
		final AutoWireSectionTransformer transformerTwo = this.createMock(AutoWireSectionTransformer.class);

		// Transformed sections
		final AutoWireSection firstTransformed = this.createMock(AutoWireSection.class);
		final AutoWireSection secondTransformed = this.createMock(AutoWireSection.class);
		...
		AbstractMatcher matcher = new AbstractMatcher() {
			...
			public boolean matches(Object[] expected, Object[] actual) {
				...
				if (this.isOriginal) {
					assertSame("Should be original", original, context.getSection());
					this.isOriginal = false;
				} else {
					assertSame("Should be first transformed", firstTransformed, context.getSection());
				}
				return true;
			}
		};
		this.recordReturn(transformerOne, transformerOne.transformAutoWireSection(null), firstTransformed, matcher);
		this.recordReturn(transformerTwo, transformerTwo.transformAutoWireSection(null), secondTransformed, matcher);

		// Record creating the transformed section
		this.recordReturn(secondTransformed, secondTransformed.getSectionName(), SECTION);
		this.recordReturn(secondTransformed, secondTransformed.getSectionSourceClassName(),
				SectionSource.class.getName());
		this.recordReturn(secondTransformed, secondTransformed.getSectionLocation(), SECTION_LOCATION);
		this.recordReturn(secondTransformed, secondTransformed.getProperties(), properties);
		this.recordReturn(this.architect,
				this.architect.addOfficeSection(SECTION, SectionSource.class.getName(), SECTION_LOCATION),
				officeSection);
		properties.configureProperties(officeSection);
	...
	public void testFlowNotLinked() throws Exception {
		...
		this.architect.addIssue("Section output 'Section:output' is not linked");
	...
	public void testInheritLinkFlows() throws Exception {
		...
		OfficeSectionOutput childOutput = this.outputs.get(CHILD).get(TEMPLATE_OUTPUT);
	...
	public void testCyclicSectionInheritanceHierarchy() throws Exception {
		...
		this.recordReturn(outputType, outputType.isEscalationOnly(), true);
		this.recordSectionOutputs(TWO);
		this.architect.addIssue("Cyclic section inheritance hierarchy ( One : Two : One : ... )");
	...
	public void testIsFlowLinked() throws Exception {
		...
		assertFalse("Section without output not linked", source.isLinked(two, "output"));
	}
	...
	public void testGetSection() throws Exception {
		...
		assertNull("Should not obtain section before added", source.getSection("SECTION"));
		...
		assertSame("Should be able to obtain section by name", section, source.getSection("SECTION"));
	}
	...
	public void testUnknownOutput() throws Exception {
		...
		this.recordSectionOutputs(TWO);
		this.architect.addIssue("Unknown section output 'One:output' to link to section input 'Two:input'");
	...
	public void testUnknownInput() throws Exception {
		...
		this.recordSectionOutputs(TWO);
		this.architect.addIssue("Unknown section input 'Two:input' for linking section output 'One:output'");
	...
	public void testOfficeObjectWithQualifiedType() throws Exception {
		...
		final ExpectedAutoWire qualifiedConnection = new ExpectedAutoWire("QUALIFIED", "QUALIFIED", Connection.class);
		final ExpectedAutoWire unqualifiedConnection = new ExpectedAutoWire(Connection.class);

		final ExpectedAutoWire qualifiedString = new ExpectedAutoWire("QUALIFIED", "QUALIFIED", String.class);
	...
	public void testNoAvailableAutoWireOfficeObject() throws Exception {
		...
		final String SECTION = "SECTION";
		final OfficeSectionObjectType objectType = this.createMock(OfficeSectionObjectType.class);
		...
		source.addAvailableOfficeObject(new AutoWire("QUALIFIED", Connection.class.getName()));
		...
		this.recordReturn(sectionType, sectionType.getOfficeSectionObjectTypes(),
				new OfficeSectionObjectType[] { objectType });
		this.recordReturn(objectType, objectType.getObjectType(), Connection.class.getName());
		this.recordReturn(objectType, objectType.getTypeQualifier(), null);
		this.recordReturn(objectType, objectType.getOfficeSectionObjectName(), "OBJECT");
		this.architect.addIssue("No available auto-wiring for Section Object OBJECT (qualifier=null, type="
	...
	public void testOfficeObjectGovernance() throws Exception {
		...
		AutoWireOfficeSource source = new AutoWireOfficeSource();
		this.addSection(source, SECTION);
		this.addGovernance(source, GOVERNANCE, "PROPERTY_NAME", "PROPERTY_VALUE");
		source.addAvailableOfficeObject(new AutoWire(Connection.class), XAResource.class);
		...
		this.recordGovernOfficeObject(GOVERNANCE, new AutoWire(Connection.class));
	...
	public void testUnusedOfficeObjectGovernance() throws Exception {
		...
		source.addAvailableOfficeObject(new AutoWire(DataSource.class), XAResource.class);
	...
	/**
	 * Ensure can provide {@link Governance}.
	 */
	public void testNothingToGovern() throws Exception {
	...
	/**
	 * Ensure can provide {@link Governance} over {@link AutoWireSection} that
	 * has no {@link OfficeSectionManagedObject}.
	 */
	public void testSectionWithoutManagedObjectsGovernance() throws Exception {
		...
		// Create and configure the source
		AutoWireOfficeSource source = new AutoWireOfficeSource();
		AutoWireSection autoWireSection = this.addSection(source, SECTION);
		AutoWireGovernance autoWireGovernance = this.addGovernance(source, GOVERNANCE, "PROPERTY_NAME",
				"PROPERTY_VALUE");
		autoWireGovernance.governSection(autoWireSection);

		// Record governance over section and managed objects within section
		this.recordOfficeSection(SECTION);
		this.recordSectionObjects(SECTION);
		this.recordSectionInputs(SECTION);
		this.recordSectionTasks(SECTION);
		...
		OfficeTeam team = this.recordTeam();
		this.architect.link(governance, team);
		OfficeSection section = this.getOfficeSection(SECTION);
	...
	/**
	 * Ensure can provide {@link Governance} over top level
	 * {@link OfficeSectionManagedObject}.
	 */
	public void testSectionManagedObjectGovernance() throws Exception {
		...
		OfficeSection section = this.getOfficeSection(SECTION);
		section.addGovernance(governance);
		this.recordGovernManagedObject(GOVERNANCE, SECTION, XAResource.class, true);
		this.recordGovernSubSections(SECTION);
	...
	/**
	 * Ensure can provide {@link Governance} over
	 * {@link OfficeSectionManagedObject}.
	 */
	public void testSubSectionManagedObjectGovernance() throws Exception {
		...
		OfficeSection section = this.getOfficeSection(SECTION);
		section.addGovernance(governance);
		this.recordGovernManagedObject(GOVERNANCE, SECTION, List.class, false);
		this.recordGovernSubSections(SECTION, SUB_SECTION);
		this.recordGovernManagedObject(GOVERNANCE, SUB_SECTION, Map.class, false);
		this.recordGovernSubSections(SUB_SECTION, SUB_SUB_SECTION);
		this.recordGovernManagedObject(GOVERNANCE, SUB_SUB_SECTION, XAResource.class, true);
	...
	public void testUnknownSectionGovernance() throws Exception {
		...
		final AutoWireSection unknownSection = this.createMock(AutoWireSection.class);
		...
		this.recordReturn(unknownSection, unknownSection.getSectionName(), UNKNOWN_SECTION);
		this.architect.addIssue("Unknown section '" + UNKNOWN_SECTION + "' to be governed by governance " + GOVERNANCE);
	...
	/**
	 * Ensure can govern transformed {@link AutoWireSection}.
	 */
	public void testTransformedSectionGovernance() throws Exception {
		...
		// Create and configure the source
		AutoWireOfficeSource source = new AutoWireOfficeSource();
		AutoWireSection section = this.addSection(source, SECTION, (seed) -> new AutoWireSectionImpl(null, seed));
		AutoWireGovernance governance = this.addGovernance(source, GOVERNANCE);
		governance.governSection(section);

		// Record govern transformed section
		this.recordOfficeSection(SECTION);
		this.recordSectionObjects(SECTION);
		this.recordSectionInputs(SECTION);
		this.recordSectionTasks(SECTION);
	...
	public void testSectionGovernance() throws Exception {
		...
		AutoWireOfficeSource source = new AutoWireOfficeSource();
		AutoWireSection section = this.addSection(source, SECTION);
		AutoWireGovernance governance = this.addGovernance(source, GOVERNANCE, "PROPERTY_NAME", "PROPERTY_VALUE");
		governance.governSection(section);
	...
	/**
	 * Ensure can provide {@link Governance} over {@link AutoWireSection} with
	 * an {@link OfficeSubSection}.
	 */
	public void testSubSectionGovernance() throws Exception {

		final String SECTION = "Section";
		final String SUB_SECTION = "SubSection";
		final String GOVERNANCE = "GOVERNANCE";
		...
		// Record governance over sub section
		this.recordOfficeSection(SECTION);
		this.recordSectionObjects(SECTION);
		this.recordSectionInputs(SECTION);
		this.recordSectionTasks(SECTION);
		...
		this.recordGovernManagedObject(GOVERNANCE, SECTION, null, false);
		this.recordGovernSubSections(SECTION, SUB_SECTION);
		this.recordGovernManagedObject(GOVERNANCE, SUB_SECTION, null, false);
		this.recordGovernSubSections(SUB_SECTION);
	 ...
	 */
	public void testGovernanceTeamByExtensionInterfaceIgnoringQualification() throws Exception {

		final String GOVERNANCE = "GOVERNANCE";

		// Create and configure the source
		AutoWireOfficeSource source = new AutoWireOfficeSource();
		source.addAvailableOfficeTeam(new AutoWire("QUALIFIED", XAResource.class.getName()));
		source.addAvailableOfficeTeam(new AutoWire(XAResource.class));
		this.addGovernance(source, GOVERNANCE, "PROPERTY_NAME", "PROPERTY_VALUE");
	...
	public void testAssignTeamForManagedObject() throws Exception {
		this.doAssignTeamTest(new AutoWire(MockManagedObject.class), true, MockTeamManagedObjectSection.class,
				new AutoWire(MockManagedObject.class));
	}
	...
	public void testUnusedTeamForManagedObject() throws Exception {
		this.doAssignTeamTest(new AutoWire(UnusedTeam.class), false, MockTeamManagedObjectSection.class,
				new AutoWire(MockManagedObject.class));
	}
	...
	public void testAssignTeamForQualifiedManagedObject() throws Exception {
		this.doAssignTeamTest(new AutoWire(MockQualifier.class, Integer.class), true,
				MockTeamQualifiedManagedObjectSection.class, new AutoWire(Integer.class));
	}
	...
	public void testUnusedTeamForQualifiedManagedObject() throws Exception {
		this.doAssignTeamTest(new AutoWire(UnusedTeam.class), false, MockTeamQualifiedManagedObjectSection.class,
	...
	public static class MockTeamQualifiedManagedObjectSection {
		...
		@ManagedObject(source = ClassManagedObjectSource.class, qualifiers = {
				@TypeQualifier(qualifier = MockQualifier.class, type = Integer.class) }, properties = {
	...
	public static class MockTeamManagedObjectDependencySection {
		...
		@ManagedObject(source = ClassManagedObjectSource.class, properties = {
				@Property(name = ClassManagedObjectSource.CLASS_NAME_PROPERTY_NAME, valueClass = MockTeamDependencyManagedObject.class) })
	...
	public static class MockTeamQualifiedManagedObjectDependencySection {
		...
		@ManagedObject(source = ClassManagedObjectSource.class, properties = {
				@Property(name = ClassManagedObjectSource.CLASS_NAME_PROPERTY_NAME, valueClass = MockTeamQualifiedDependencyManagedObject.class) })
		MockTeamDependencyManagedObject managedObject;
	 ...
	 */
	private void doAssignTeamTest(final AutoWire teamAutoWire, final boolean isUseTeam, Class<?> sectionClass,
		...
		this.recordOfficeSection(sectionClass, new TeamAssigner() {
			...
			@Override
			public void recordAssignTeam(String taskName, OfficeSectionTask task, OfficeTaskType taskType,
					OfficeTaskType actualTaskType) {
				...
				if (assignedTeam == null) {
					...
					if (this.defaultTeam == null) {
						this.defaultTeam = AutoWireOfficeSourceTest.this.recordTeam();
					}
					assignedTeam = this.defaultTeam;
				}

				// Assign the team
				TaskTeam responsibleTeam = AutoWireOfficeSourceTest.this.createMock(TaskTeam.class);
	...
	private OfficeTeam recordTeam() {
		...
		if (this.team == null) {
			...
			this.recordReturn(this.architect, this.architect.addOfficeTeam("team"), this.team);
		}
	...
	private OfficeTeam recordTeam(AutoWire autoWire) {
		...
		this.recordReturn(this.architect, this.architect.addOfficeTeam(autoWire.getQualifiedType()), officeTeam);
		return officeTeam;
	}
	 ...
	 */
	private AutoWireSection addSection(AutoWireOfficeSource source, Class<?> sectionClass) {

		final String SECTION_NAME = sectionClass.getSimpleName();

		// Add the section
		AutoWireSection section = source.addSection(SECTION_NAME, ClassSectionSource.class.getName(),
	 ...
	 */
	private OfficeSection recordOfficeSection(Class<?> sectionClass, TeamAssigner assigner) {

		final String SECTION_NAME = sectionClass.getSimpleName();

		// Obtain the properties
		PropertyList properties = this.sectionProperties.get(SECTION_NAME);
		assertNotNull("Section " + SECTION_NAME + " should be added", properties);

		// Obtain the actual section type for configuring recording
		OfficeSectionType actualType = SectionLoaderUtil.loadOfficeSectionType(SECTION_NAME, ClassSectionSource.class,
				sectionClass.getName());

		// Record adding the office section
		OfficeSection officeSection = this.createMock(OfficeSection.class);
		this.recordReturn(this.architect, this.architect.addOfficeSection(SECTION_NAME,
				ClassSectionSource.class.getName(), sectionClass.getName()), officeSection);
		this.sections.put(SECTION_NAME, officeSection);

		// Record obtaining the office section type
		OfficeSectionType officeSectionType = this.createMock(OfficeSectionType.class);
		this.recordReturn(this.context, this.context.loadOfficeSectionType(SECTION_NAME,
				ClassSectionSource.class.getName(), sectionClass.getName(), properties), officeSectionType);
		this.sectionTypes.put(SECTION_NAME, officeSectionType);

		// Record obtaining the office objects
		OfficeSectionObjectType[] actualObjectTypes = actualType.getOfficeSectionObjectTypes();
		String[] objectNames = (Arrays.asList(actualObjectTypes)).stream().map(t -> t.getOfficeSectionObjectName())
				.toArray(String[]::new);
		OfficeSectionObjectType[] objectTypes = this.createSectionItems(SECTION_NAME, OfficeSectionObjectType.class,
				this.objectTypes, objectNames);
		this.recordReturn(officeSectionType, officeSectionType.getOfficeSectionObjectTypes(), objectTypes);
		for (int i = 0; i < objectTypes.length; i++) {
			...
			OfficeSectionObject sectionObject = this.createMock(OfficeSectionObject.class);
			this.recordReturn(officeSection, officeSection.getOfficeSectionObject(objectName), sectionObject);
			...
			this.recordReturn(this.architect, this.architect.addOfficeObject(
					new AutoWire(typeQualifier, objectTypeName).getQualifiedType(), objectTypeName), officeObject);
		...
		String[] inputNames = (Arrays.asList(actualInputTypes)).stream().map(t -> t.getOfficeSectionInputName())
				.toArray(String[]::new);
		OfficeSectionInputType[] inputTypes = this.createSectionItems(SECTION_NAME, OfficeSectionInputType.class,
				this.inputTypes, inputNames);
		OfficeSectionInput[] inputs = this.createSectionItems(SECTION_NAME, OfficeSectionInput.class, this.inputs,
				inputNames);
		...
		String[] taskNames = (Arrays.asList(actualTaskTypes)).stream().map(t -> t.getOfficeTaskName())
				.toArray(String[]::new);
		OfficeTaskType[] taskTypes = this.createSectionItems(SECTION_NAME, OfficeTaskType.class, this.taskTypes,
				taskNames);
		OfficeSectionTask[] tasks = this.createSectionItems(SECTION_NAME, OfficeSectionTask.class, this.tasks,
				taskNames);
		...
		OfficeSectionOutputType[] actualOutputTypes = actualType.getOfficeSectionOutputTypes();
		String[] outputNames = (Arrays.asList(actualOutputTypes)).stream().map(t -> t.getOfficeSectionOutputName())
				.toArray(String[]::new);
		OfficeSectionOutputType[] outputTypes = this.createSectionItems(SECTION_NAME, OfficeSectionOutputType.class,
				this.outputTypes, outputNames);
		this.recordReturn(officeSectionType, officeSectionType.getOfficeSectionOutputTypes(), outputTypes);
		...
		for (int i = 0; i < outputTypes.length; i++) {
			...
			this.recordReturn(officeSection, officeSection.getOfficeSectionOutput(outputName), sectionOutput);

			// Record adding the office output
			String outputTypeName = actualOutputType.getArgumentType();
			this.recordReturn(outputType, outputType.getArgumentType(), outputTypeName);
			OfficeOutput officeOutput = this.createMock(OfficeOutput.class);
			this.recordReturn(this.architect, this.architect.addOfficeOutput(outputName, outputTypeName), officeOutput);
	...
	private static interface TeamAssigner {
		 ...
		 */
		void recordAssignTeam(String taskName, OfficeSectionTask task, OfficeTaskType taskType,
				OfficeTaskType actualTaskType);
	}
	 ...
	 * @return {@link AutoWireSection}.
	 */
	private <A extends AutoWireSection> AutoWireSection addSection(AutoWireOfficeSource source, String sectionName,
		...
		if (factory == null) {
			section = source.addSection(sectionName, SectionSource.class.getName(), sectionLocation);
		} else {
			section = source.addSection(sectionName, SectionSource.class.getName(), sectionLocation, factory);
		}
	 ...
	 * @return {@link AutoWireSection}.
	 */
	private AutoWireSection addSection(AutoWireOfficeSource source, String sectionName, String... propertyNameValues) {
	 ...
	 * @return {@link OfficeSection}.
	 */
	private void recordOfficeSection(String sectionName, String... nameValuePairs) {
		assertNull("Already section by name " + sectionName, this.sections.get(sectionName));
		...
		OfficeSectionType sectionType = this.createMock(OfficeSectionType.class);
		this.recordReturn(this.context, this.context.loadOfficeSectionType(sectionName, SectionSource.class.getName(),
				sectionLocation, properties), sectionType);
		this.sectionTypes.put(sectionName, sectionType);

		// Record creating the section
		OfficeSection section = this.createMock(OfficeSection.class);
		this.recordReturn(this.architect,
				this.architect.addOfficeSection(sectionName, SectionSource.class.getName(), sectionLocation), section);
	 ...
	 */
	private AutoWireGovernance addGovernance(AutoWireOfficeSource source, String governanceName,
			String... propertyNameValues) {

		// Add the governance
		AutoWireGovernance governance = source.addGovernance(governanceName, GovernanceSource.class.getName());
		...
		this.governanceProperties.put(governanceName, governance.getProperties());
	 ...
	 *            Extension interface.
	 */
	private OfficeGovernance recordGovernance(String governanceName, Class<?> extensionInterface) {
		assertNull("Already Governance by name " + governanceName, this.governances.get(governanceName));

		// Record creating the Governance
		OfficeGovernance governance = this.createMock(OfficeGovernance.class);
		this.recordReturn(this.architect,
				this.architect.addOfficeGovernance(governanceName, GovernanceSource.class.getName()), governance);
	 ...
	 *            {@link Governance}.
	 */
	private void recordGovernSections(String governanceName, String... sectionNames) {

		// Obtain the governance
		OfficeGovernance officeGovernance = this.governances.get(governanceName);
	 ...
	 */
	private void recordGovernManagedObject(String governanceName, String sectionName, Class<?> extensionInterface,
			boolean isGovern) {

		// Obtain the section and its type
		OfficeSubSection section = this.getOfficeSubSection(sectionName);
		OfficeSubSectionType sectionType = this.getOfficeSubSectionType(sectionName);

		// Record section managed object
		if (extensionInterface == null) {
			// No managed object for section
			this.recordReturn(sectionType, sectionType.getOfficeSectionManagedObjectTypes(),
					new OfficeSectionManagedObjectType[0]);

		} else {
			// Managed object for section
			final String MO_NAME = "MO_NAME";
			OfficeSectionManagedObjectType moType = this.createMock(OfficeSectionManagedObjectType.class);
			this.recordReturn(sectionType, sectionType.getOfficeSectionManagedObjectTypes(),
					new OfficeSectionManagedObjectType[] { moType });
			this.recordReturn(moType, moType.getSupportedExtensionInterfaces(), new Class<?>[] { extensionInterface });
			...
			if (isGovern) {
				...
				this.recordReturn(moType, moType.getOfficeSectionManagedObjectName(), MO_NAME);
				OfficeSectionManagedObject mo = this.createMock(OfficeSectionManagedObject.class);
				this.recordReturn(section, section.getOfficeSectionManagedObject(MO_NAME), mo);
	 ...
	 *            {@link Governance}.
	 */
	private void recordGovernOfficeObject(String governanceName, AutoWire... autoWiring) {
		OfficeGovernance governance = this.governances.get(governanceName);
		for (AutoWire autoWire : autoWiring) {
			OfficeObject officeObject = this.dependencies.get(autoWire.getQualifiedType());
	 ...
	 */
	private void recordGovernSubSections(String sectionName, String... subSectionNames) {
		OfficeSubSection section = this.getOfficeSubSection(sectionName);
		OfficeSubSectionType sectionType = this.getOfficeSubSectionType(sectionName);
		...
		this.recordReturn(sectionType, sectionType.getOfficeSubSectionTypes(), subSectionTypes);
		for (int i = 0; i < subSectionTypes.length; i++) {
			String subSectionName = subSectionNames[i];
			OfficeSubSectionType subSectionType = subSectionTypes[i];
			this.recordReturn(subSectionType, subSectionType.getOfficeSectionName(), subSectionName);
			OfficeSubSection subSection = this.getOfficeSubSection(subSectionName);
	...
	private void recordSectionInputs(String sectionName, String... inputNames) {
		...
		OfficeSectionInput[] inputs = this.createSectionItems(sectionName, OfficeSectionInput.class, this.inputs,
				inputNames);
		OfficeSectionInputType[] inputTypes = this.createSectionItems(sectionName, OfficeSectionInputType.class,
				this.inputTypes, inputNames);
		this.recordReturn(sectionType, sectionType.getOfficeSectionInputTypes(), inputTypes);
		for (int i = 0; i < inputNames.length; i++) {
			this.recordReturn(inputTypes[i], inputTypes[i].getOfficeSectionInputName(), inputNames[i]);
			this.recordReturn(section, section.getOfficeSectionInput(inputNames[i]), inputs[i]);
		}
	}
	...
	private void recordSectionTasks(String sectionName) {
		OfficeSectionType sectionType = this.getOfficeSectionType(sectionName);
		this.recordReturn(sectionType, sectionType.getOfficeTaskTypes(), new OfficeTaskType[0]);
	}
	...
	private void recordSectionOutputs(String sectionName, String... outputNames) {
		...
		OfficeSectionOutput[] outputs = this.createSectionItems(sectionName, OfficeSectionOutput.class, this.outputs,
				outputNames);
		OfficeSectionOutputType[] outputTypes = this.createSectionItems(sectionName, OfficeSectionOutputType.class,
				this.outputTypes, outputNames);
		this.recordReturn(sectionType, sectionType.getOfficeSectionOutputTypes(), outputTypes);
		for (int i = 0; i < outputNames.length; i++) {
			this.recordReturn(outputTypes[i], outputTypes[i].getOfficeSectionOutputName(), outputNames[i]);
			this.recordReturn(section, section.getOfficeSectionOutput(outputNames[i]), outputs[i]);
		}
	}
	 ...
	 */
	private OfficeSectionOutputType recordSectionUnlinkedOutput(String sectionName, String outputName) {
		OfficeSectionType sectionType = this.getOfficeSectionType(sectionName);
		OfficeSectionOutputType outputType = this.createMock(OfficeSectionOutputType.class);
		this.recordReturn(sectionType, sectionType.getOfficeSectionOutputTypes(),
				new OfficeSectionOutputType[] { outputType });
		this.recordReturn(outputType, outputType.getOfficeSectionOutputName(), outputName);
		return outputType;
	}
	 ...
	 */
	private void recordSectionObjects(String sectionName, ExpectedAutoWire... autoWiring) {
		...
		OfficeSectionObject[] objects = this.createSectionItems(sectionName, OfficeSectionObject.class, this.objects,
				objectNames);
		OfficeSectionObjectType[] objectTypes = this.createSectionItems(sectionName, OfficeSectionObjectType.class,
				this.objectTypes, objectNames);
		...
		this.recordReturn(sectionType, sectionType.getOfficeSectionObjectTypes(), objectTypes);
		...
		for (int i = 0; i < autoWiring.length; i++) {
			...
			this.recordReturn(objectType, objectType.getTypeQualifier(), usedQualifier);
			this.recordReturn(objectType, objectType.getOfficeSectionObjectName(), objectName);

			// Record obtaining the object
			this.recordReturn(section, section.getOfficeSectionObject(objectName), object);
			...
			if (dependency == null) {
				...
				this.recordReturn(this.architect, this.architect.addOfficeObject(usedName, usedType), dependency);
	 ...
	 */
	private void recordSubSections(String sectionName, String... subSectionNames) {
		...
		for (int i = 0; i < subSectionNames.length; i++) {
			...
			OfficeSubSectionType subSectionType = this.subSectionTypes.get(subSectionName);
		...
		for (int i = 0; i < subSectionNames.length; i++) {
			...
			assertNull("Sub section already registered " + subSectionName, this.subSections.get(subSectionName));
			OfficeSubSection subSection = this.createMock(OfficeSubSection.class);
			...
			this.recordReturn(subSectionType, subSectionType.getOfficeTaskTypes(), new OfficeTaskType[0]);
		}
	}
	 ...
	 */
	private void recordEscalation(Class<? extends Throwable> escalationType, String sectionName, String inputName) {
		OfficeEscalation escalation = this.createMock(OfficeEscalation.class);
		this.recordReturn(this.architect, this.architect.addOfficeEscalation(escalationType.getName()), escalation);
		OfficeSectionInput sectionInput = this.inputs.get(sectionName).get(inputName);
	...
	private void recordStartupFlow(String sectionName, String inputName) {
		...
		this.recordReturn(this.architect, this.architect.addOfficeStart(sectionName + "-" + inputName), start);
		OfficeSectionInput sectionInput = this.inputs.get(sectionName).get(inputName);
	...
	private <T> T[] createSectionItems(String sectionName, Class<T> itemType, Map<String, Map<String, T>> items,
			String... itemNames) {
		assertNull("Already obtained  " + itemType.getSimpleName() + " for section " + sectionName,
				items.get(sectionName));
	...
	private static class ExpectedAutoWire {
		 ...
		 *            Type.
		 */
		public ExpectedAutoWire(String requestedQualifier, String usedQualifier, Class<?> type) {
			this.requestedAutoWire = new AutoWire(requestedQualifier, type.getName());
