...
public class ClassGovernanceSource extends
		...
		GovernanceSourceService<Object, Indexed, ClassGovernanceSource> {
	...
	protected void loadMetaData(MetaDataContext<Object, Indexed> context)
		...
		Class<Object> extensionInterface = (Class<Object>) this
				.validateMethodAndLoadEscalations(governMethod, true,
		...
		this.validateMethodAndLoadEscalations(enforceMethod, true,
				Enforce.class, 0, loadedEscalations, context);

		// Obtain the disregard method
		Method disregardMethod = this.getMethod(clazz, Disregard.class);
		this.validateMethodAndLoadEscalations(disregardMethod, false,
				Disregard.class, 0, loadedEscalations, context);

		// Provide the governance factory
		context.setGovernanceFactory(new ClassGovernanceFactory(clazz,
				governMethod, enforceMethod, disregardMethod));
	}
	 ...
	 *            {@link Annotation}.
	 * @return {@link Method} with the {@link Annotation} or <code>null</code>
	 *         if no {@link Method}.
	 */
			...
			if (method.isAnnotationPresent(annotationType)) {
				...
				if (annotatedMethod != null) {
					throw new IllegalStateException(
							"Only one method may be annotated with "
	 ...
	 *            {@link Method}.
	 * @param annotationType
	 *            {@link Annotation} for the {@link Method}.
	 * @param numberOfParameters
	 *            Number of parameters expected for the {@link Method}.
	 * @param loadedEscalations
	 *            Already loaded {@link Escalation} instances.
	 * @param context
	 *            {@link MetaDataContext}.
	 * @return Parameter types for the {@link Method}.
	 */
	private Class<?>[] validateMethodAndLoadEscalations(Method method,
			boolean isMethodRequired,
			Class<? extends Annotation> annotationType, int numberOfParameters,
			Set<Class<?>> loadedEscalations,
		...
		if (method == null) {
			if (isMethodRequired) {
				// Method required
				throw new IllegalStateException(
						"A method must be annotated with @"
								+ annotationType.getSimpleName());
			} else {
				// No method to check
				return null;
		...
		if (methodParams.length != numberOfParameters) {
			throw new IllegalStateException(annotationType.getSimpleName()
					+ " method must have " + numberOfParameters + " parameter"
					+ (numberOfParameters != 1 ? "s" : 0));
		}
