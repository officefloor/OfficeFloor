...
public class ServerInputStreamImpl extends ServerInputStream {
	 ...
	 */
	public ServerInputStreamImpl(Object lock, Serializable stateMomento) throws IllegalArgumentException {
		this(lock);

		// Ensure state momento is valid
		if (!(stateMomento instanceof StateMomento)) {
			throw new IllegalArgumentException("Invalid momento for " + ServerInputStream.class.getSimpleName());
		}
		...
		this.available = calculateAvailable(this.headBuffer, this.currentBufferIndex, this.isFurtherData);
	}
	...
	public Serializable exportState() throws NotAllDataAvailableException {
		...
		synchronized (this.lock) {
			...
			if (this.isFurtherData) {
				throw new NotAllDataAvailableException(ServerInputStream.class.getSimpleName()
	...
	/**
	 * Browse {@link InputStream} implementation.
	 */
	private class BrowseInputStreamImpl extends InputStream {
		...
		public int available() throws IOException {
			...
			synchronized (ServerInputStreamImpl.this.lock) {
				...
				return calculateAvailable(this.currentBuffer, this.currentBufferIndex,
	 ...
	 */
	private static int calculateAvailable(ReadBuffer currentBuffer, int currentBufferIndex, boolean isFurtherData) {

		// Determine if data is available on current buffer
		if ((currentBuffer != null) && (currentBufferIndex > currentBuffer.endIndex)) {
