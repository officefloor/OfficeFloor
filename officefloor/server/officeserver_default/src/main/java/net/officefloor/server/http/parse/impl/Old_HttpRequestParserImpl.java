 ...
 */
public class Old_HttpRequestParserImpl implements HttpRequestParser {
	 ...
	 */
	public Old_HttpRequestParserImpl(int maxHeaderCount, int maxTextLength, long maxEntityLength) {
	...
	public boolean parse(ByteBuffer buffer) throws IOException, HttpRequestParseException {
		...
		if (this.contentLength > 0) {
			...
			if (remainingBytes > requiredBytes) {
				...
				// this.entityStream.inputData(buffer,
				// this.nextByteToParseIndex, endIndex, false);
				this.nextByteToParseIndex = endIndex + 1; // after bytes read

			} else {
				// Consume all of the bytes
				// this.entityStream.inputData(buffer,
				// this.nextByteToParseIndex, (buffer.position() - 1),
