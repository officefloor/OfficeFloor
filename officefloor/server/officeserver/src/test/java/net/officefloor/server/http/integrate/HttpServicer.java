...
public class HttpServicer {
	...
	public void service(ServerHttpConnection connection) throws Throwable {
		...
		TestCase.assertNotNull("Must have local host", actualLocalAddress.getHostName());
		TestCase.assertEquals("Incorrect local port", expectedLocalPort, actualLocalAddress.getPort());

		// Validate the remote address (not same port as local)
		InetSocketAddress actualRemoteAddress = connection.getRemoteAddress();
		TestCase.assertNotNull("Must have remote host", actualRemoteAddress.getHostName());
		TestCase.assertTrue("Remote address port to be different to local address port",
		...
		System.out.println(this.getClass().getSimpleName() + " serving request: " + request.getMethod() + " "
