 ...
 */
public class HttpApplicationLocationManagedObjectSourceTest extends OfficeFrameTestCase {
	...
	public void testCopyProperties() {
		...
		this.recordCopyProperty(source, target, "http.host", "test.officefloor.net");
		...
		this.recordCopyProperty(source, target, "cluster.http.host", "node.officefloor.net");
		...
		HttpApplicationLocationManagedObjectSource.copyProperties(source, target);
	 ...
	 */
	private void recordCopyProperty(SourceProperties source, PropertyConfigurable target, String name, String value) {
	...
	public void testSpecification() {
		ManagedObjectLoaderUtil.validateSpecification(HttpApplicationLocationManagedObjectSource.class);
	}
	...
	public void testType() {
		...
		// Validate type
		ManagedObjectLoaderUtil.validateManagedObjectType(type, HttpApplicationLocationManagedObjectSource.class);
	}
	...
	public void testLoadFromDefaults() {
		...
		// Validate location
		assertLocation(this.hostName, 7878, 7979, null, this.hostName, 7878, 7979, location);
	}
	...
	public void testLoadForSingleInstance() {
		...
		HttpApplicationLocation location = this.loadHttpApplicationLocation("http.host", "application.officefloor.net",
				"http.port", "8888", "https.port", "8989", "http.context.path", "/test");

		// Validate location
		assertLocation("application.officefloor.net", 8888, 8989, "/test", this.hostName, 8888, 8989, location);
	}
	...
	public void testLoadWithinCluster() {
		...
		// Load for single instance configuration
		HttpApplicationLocation location = this.loadHttpApplicationLocation("http.host", "application.officefloor.net",
				"http.port", "8888", "https.port", "8989", "http.context.path", "/test", "cluster.http.host",
				"node.officefloor.net", "cluster.http.port", "2222", "cluster.https.port", "2323");

		// Validate location
		assertLocation("application.officefloor.net", 8888, 8989, "/test", "node.officefloor.net", 2222, 2323,
				location);
	}
	...
	public void testCanonicalContextPath() {
		...
		assertEquals("/context", this.loadHttpApplicationLocation("http.context.path", "context").getContextPath());

		// Ensure transform to canonical path
		assertEquals("/context",
				this.loadHttpApplicationLocation("http.context.path", "//./something/../context/").getContextPath());

		// Ensure root context path results in no path
		assertNull(this.loadHttpApplicationLocation("http.context.path", " / ").getContextPath());
	}
	 ...
	 */
	private HttpApplicationLocation loadHttpApplicationLocation(String... parameterNameValues) {
		try {
			...
			user.mapDependency(Dependencies.SERVER_HTTP_CONNECTION, this.connection);
			ManagedObject managedObject = user.sourceManagedObject(source);
			assertTrue("Should be co-ordinating", (managedObject instanceof CoordinatingManagedObject));

			// Obtain the object
			Object object = managedObject.getObject();
			assertTrue("Should be HttpApplicationLocation", (object instanceof HttpApplicationLocation));
	 ...
	 */
	private static void assertLocation(String domain, int httpPort, int httpsPort, String contextPath, String hostName,
			int clusterHttpPort, int clusterHttpsPort, HttpApplicationLocation location) {
		...
		if (contextPath == null) {
			assertNull("Should not have context path", location.getContextPath());
		} else {
			assertEquals("Incorrect context path", contextPath, location.getContextPath());
		}
		assertEquals("Incorrect cluster host", hostName, location.getClusterHostName());
		assertEquals("Incorrect cluster HTTP port", clusterHttpPort, location.getClusterHttpPort());
		assertEquals("Incorrect cluster HTTPS port", clusterHttpsPort, location.getClusterHttpsPort());
	}
