...
\textbf{TODO change for title} This paper presents OfficeFloor and its
underlying Job Based Architecture to enable isolating application level performance bottlenecks rather than relying
on server tuning to achieve more consistent performance for servicing dynamic
content. As tuning is critical for performance of web
servers~\cite{tuning-important,tuning-os-important}, I find there are trade-offs
in both developing and tuning Internet based applications (e.g. pipelining
incuring high thread context switching or thread-per-request incuring large
server footprint).  Rather than trying to optimise the choice and tuning of the
web server I propose that isolating application level performance bottlenecks to
...
cached) the servicing of the request may be executed completely by one thread
and not incur a thread context switch.
...
configuration editors).  Furthermore, it can reduce arising bottlenecks from
human configuration/coding errors based on task type especially as tasks may
change their processing characteristics over the evolution of the application
and I expect their changing performance bottlenecks can be better identified
through task dependencies than task type.
...
first time that the patterns of pipeline, continuations, reactor and inversion
of control have been used with the abstraction of a thread as a transitive
...
performance bottleneck.  Comparisons against other web server architectures,
