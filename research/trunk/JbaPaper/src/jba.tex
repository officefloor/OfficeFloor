...
functions containing application level performance bottlenecks to not consume
all threads and subsequently not starve other unrelated functions of a thread.
...
request throughput comparison of the OfficeFloor implementation against other
...
thread-per-request web servers for servicing requests for differing
...
enable isolating application level performance bottlenecks rather than relying
on server tuning to achieve more consistent performance. As tuning is critical
for performance of web servers~\cite{tuning-important,tuning-os-important} along
with maintaining a small server footprint~\cite{low-server-footprint}, I find
there are trade-offs that require significant involvement by developers in both
developing and tuning Internet based applications (e.g.
...
tuning of the web server I propose that isolating application level performance
bottlenecks to better decouple their effects will provide more consistent
performance.  Furthermore, through the explanation of the Job Based Architecture
I will demonstrate how a thread can be a parameter to a function for isolating
...
uses the dependency information of the next job to determine if the next job may
be executed by the current thread.  In
...
executed by the thread from the Database thread pool.  The Write HTTP response
job may however be completed by the Network thread pool if the send socket
buffer is full.
...
database call) is assigned to a particular thread pool and does not affect
other requests not subject to the bottleneck. Furthermore, should there be no
performance bottlenecks to isolate (e.g. database data is cached) the servicing
of the request may be executed by one thread allowing the Job Based Architecture
to behave like an event-based architecture in this case.
...
also enables providing tools to aid developers (such as OfficeFloor's graphical
configuration editors) and reduces arising bottlenecks from human
configuration/coding errors based on job type especially as jobs may change
their dependencies and subsequently processing characteristics over the
...
have been used with the abstraction of a thread as a transitive dependency to
achieve a lower context switching pipeline concurrency model that focuses on
application level performance bottleneck isolation.
...
important to overall server performance.
...
Isolating application level performance bottlenecks by the thread being a
transitive dependency of the task has shown to provide more consistent
performance across differing load profiles by avoiding creating excessive
threads, avoiding thread starvation and avoiding excessive thread context
switching than thread-per-request web servers for dynamic content.  Furthermore,
in many load profiles isolating the bottleneck provides improved throughput
performance.
...
concurrency model.  Ease of development will be explored.  Also, expanding to
other Internet services other than web servers is another area for future work.
