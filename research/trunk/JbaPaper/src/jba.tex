...
\IEEEauthorblockA{http://officefloor.net}}
...
from a database would be as Table~\ref{tab:example_request_jobs}.
...
one of the following requests a established with the load profile of
connections varied:
\begin{IEEEitemize}
...
The servers a run on a two 1.4GHz core machine with 3GB RAM while the test
client run on are three 2.2GHz core machine with 6GB RAM.  Server and client are
