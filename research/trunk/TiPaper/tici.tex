...
%% TODO: what is the pain point?  or change to Intent with Problem description.

\textbf{How to loosely couple invoking a component?}
...
execution by another thread of control (with the possibility to execute
components concurrently).  In both patterns, they tightly couple the invocation as
the \textsc{thread-per-request} pattern must have the invoker provide the thread
of control and the \textsc{proactor} pattern must have the invoker construct the
invoked component.
...
\textbf{TODO: when not to use the pattern?}
...
component, register it for execution and then handle completion of the component
which may include exceptions.  Furthermore, handling completion of multiple
concurrently executing components may not be intuitive for many developers.
When concurrency is not necessary synchronously invoking components, via the
...
Also, the synchronous nature of \textsc{thread-per-request} pattern also imposes
...
invoked, albeit they may be executed concurrently.  The invoker must construct
the component to be asynchronously invoked and therefore this does not provide
the indirection to control invoking a different component.

Furthermore, the framework (e.g. Web Server) predefines the component's
interface to enable invoking the components.  However, components should be able
to invoke other components for re-use.  To enable re-use of these components the
interface to invoke other components should be standardised.
...
%% TODO: (use sequence to show Proactor pattern participants) In consequences part it is said that Continuation Injection pattern provides the algorithms implementing Proactor pattern's Initiator, Completion Dispatcher and Completion Handler participants. From the solution, I was unable to tell that the pattern provided any algorithms. I am not expert in this domain, but at least for me this part was unclear. 
...
Control pattertn, state can be shared by dependencies so there is no need for a
return value from the \texttt{Future}.}.  The \texttt{Future} would utilise the
...
reduce runtime errors and therefore may restrict some mapping changes.  An
adapting Asynchronous Operation may however be mapped in to remove this
restriction by transforming the parameter to the necessary type for the handling
Asynchronous Operation.}.  Changing the handling Asynchronous Operations enables
...
also have qualifying attributes that further scope dependency
contexts\footnote{OfficeFloor \cite{officefloor} utilises attributes on the
continuation mapping configuration to scope thread contexts within a request
(process) context.}.
...
continuation to service requests.}.
...
it adheres to the principles of the Actor Model.  The Asynchronous Operation is
the actor.  The asynchronous communication between Asynchronous Operations
decouples the continuation argument (message) from the sending Asynchronous
Operation (actor).  The \texttt{operationId} provides an address for an
Asynchronous Operation (actor).  The provided \texttt{continuationId}s restricts
the Asynchronous Operations (actors) that may be used.
