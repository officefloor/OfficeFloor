...
The \texttt{DependencyInjectionFactory} (Fig
...
\caption{Sequence diagram of invoking \texttt{doContinuation(\ldots)} (rotated to fit on page).}
...
component invokes the \texttt{handleException(\ldots)} method with the
...
method with the caught exception.}.  The \texttt{handleException(\ldots)} is
implemented by mapping the exception type to a \texttt{continuationId} and
...
share state between the isolated \texttt{DependencyContext}s, the isolated
...
to the \texttt{Component} instance (Fig
\ref{fig:ContinuationInjectionInterfaces}) into an Inversion of Control pattern
framework.  This allows the framework to re-use components not specifically
built for it\footnote{As an example, OfficeFloor \cite{officefloor} provides an
adapter to run a JEE Servlet as a component (Task).}.
