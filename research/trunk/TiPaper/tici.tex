...
with the \textsc{dependency injection} pattern to realise an Inversion of
...
\end{verbatim}
\caption{Continuation Injection interfaces.}
...
\texttt{doContinuation(\ldots)} method (Fig
...
dependencies (state) for the invoked component.
...
\end{figure}
\footnotetext{\texttt{componentId} may be an integer identifier for fast array look ups rather than a string.}
...
it will be handled by a configurated continuation to another component with the
exception as the continuation argument.  The \texttt{ComponentContext} provides
means to retrieve state via \textsc{dependency injection} and invoke other
components via Continuation Injection.  The \texttt{DependencyInjectionFactory}
is passed the continuation argument to use in constructing the component.
...
\textbf{TODO: fix up reference to component (from Asynchronous Operation)}
...
\section{TODO: Extra to include back}

\texttt{DependencyInjectionFactory}\footnote{A rudimentary
...
\texttt{getDependency(\ldots)} method\footnote{The
\texttt{getDependency(\ldots)} method provides retrieval of the extrinsically
defined dependencies for the Asynchronous Operation.  An example implementation
of this method would be calling Spring's \cite{spring}
\texttt{getBean(\ldots)} method.}

\footnote{To further decouple the
...
\ref{fig:ContinuationInjectionInterfaces}) to the context specific identifier for the
