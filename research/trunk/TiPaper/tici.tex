...
Popular modern web servers (Netcraft November 2012 survey) serve dynamic web
content by CGI/FastCGI with for example PHP scripts, Microsoft's HTTP.sys/WAS,
and JEE Servlets. These servers use the \textsc{thread-per-request} pattern that
is intuitive for developers to use but suffer from the thread starvation of a
slow task by only using a single thread pool.
...
downstream systems to be asynchronous non-blocking I/O.  The responsible thread
pools of OfficeFloor enables prioritisation of blocking I/O for more intuitive
development.
