...
occuring within the office.  Using the proven patterns from the office has
...
another \texttt{Task} with a different thread (stack).  The benefit of providing
hand-offs through a \texttt{TaskContext} is that the \texttt{Task} is free to depend on as many
hand-offs as necessary, and not what the caller provides.  It also means that as
the application evolves the \texttt{Task} encapsulates potential changes requiring
...
modularises the Task Orchestration into sections.  Each section provides
hand-off mapping configuration between \texttt{Task}s and other contained
sections (\textsc{process defines itself}).  As sections may contain other
sections, it creates a bottom-up approach to configuration.  The lower-level
layer sections are configured first and then the higher-level layer sections
configure these lower-level layer sections together\footnote{At the top level,
OfficeFloor configures the sections within an office.  The office configuration
enables weaving \texttt{Task}s as aspects into the contained sections
(identified as the \textsc{admistration} pattern).  The office configuration
also provides additional management over dependencies, such as demarcating
transactions (identified as the \textsc{governance} pattern).  These patterns
will be discussed in the future work of usage patterns of OfficeFloor to build
applications.}.
...
\texttt{Task}s, developers may start by building the \texttt{Task}s first.  As
