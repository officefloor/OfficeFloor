...
Inverting the control by allowing the lower \textsc{layers} to define the
variation points to be implemented by the higher \textsc{layers} will allow a
bottom-up design.  This will allow application architecture to be evolved
upwards rather than designed in a top down approach.  This architecture will
better suit development methodologies, such as Agile, where architecture is
evolved upwards.
...
Components provide discrete re-usable pieces of functionality (application
behaviour) that may be developed by different developers.  The \textsc{explicit
interfaces} of the components allows for exchanging components at the different
\textsc{layers} of the architecture \cite{ioc}.
...
Within a \texsc{layers} architecture having variation points defined by
\textsc{template method}s requires refactoring of the \textsc{template method}
to increase the variability of the lower \textsc{layers}.  Increasing the
...
\textsc{dependency injection} enables the lower \textsc{layer} component to
specify the state (objects) it requires.  When \textsc{dependency injection} is
used with \textsc{continuation injection}, the lower \textsc{layer} component
specifies its required state (objects) via \texttt{stateId}s.  As the framework
will provide the necessary objects (state) to the component, the lower
\textsc{layer} component may specify as many dependencies as necessary.
The invoking higher \textsc{layer} components need only provide the single
optionally used argument.  This leaves the lower \textsc{layer} component to
control which objects (dependecies/state) it may depend on.
...
\textbf{TODO} Bottom up design and evolution of systems.  This is because all
components are peers and not constructed into a hierarchy.

\textbf{TODO} Components themselves define the variation points and is not
constrained by a higher \textsc{layers}.  This puts the control of
variation with the component rather than the framework.
