...
Furthermore, the office patterns decouple the collaboration of methods to enable
bottom-up development of applications.  Businesses evolve their processes and
subsequently do not take a top-down approach \textbf{TODO: reference?}.  The
office patterns reflect this upwards evolution of the business.  Re-using the
office patterns within software enables a similar bottom-up approach to evolve
the application.
...
While there is extensive literature on effective organisational structure for
businesses \cite{organisational-behaviour} \textbf{TODO: more references},
many of the people aspects do not need consideration within software.  Within
software the thread (person) will undertake all methods (tasks) exactly as
specified.  Therefore, in following the above analogies, the grouping of threads
into thread pools is similar to grouping people into functional
teams\footnote{The use of the term team within this paper more closely aligns to
staff of a functional department within the literature.  However, the auther
finds the term department is perceived as more than just the staff.  The auther,
therefore, uses the term team as a better analogy to focus the reader on a
thread pool being a group of theads (people) with the goal of completing a set
of related methods (tasks).  Also, the \textsc{do it yourself} pattern does
enable a thread pool (team) to execute methods (tasks) outside its responsible
category (functional departmentalisation).}.  Each team (thread pool) is
responsible for undertaking a specific set of tasks (methods).
