...
\texttt{BeanFactory} to implement \texttt{createOperation(\ldots)} by calling
\texttt{getBean(\ldots)}.  A more functional example implementation would
utilize Spring's \texttt{ApplicationContext} to manage the context for the
components and their dependencies.  Note Spring's \texttt{BeanFactory}
implementation would, however, need to be enhanced to enable a configuration
attribute to identify a dependency to be the parameter supplied for the creation
of the bean.}
