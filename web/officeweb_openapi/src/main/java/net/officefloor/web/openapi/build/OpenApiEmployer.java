package net.officefloor.web.openapi.build;
...
/**
 * Employer for the {@link net.officefloor.web.openapi.OpenApiArchitect}.
 */
public class OpenApiEmployer {

    /**
     * Employs the {@link OpenApiArchitect}.
     *
     * @param office        {@link OfficeArchitect}.
     * @param web           {@link WebArchitect}.
     * @param security      {@link HttpSecurityArchitect} and may be <code>null</code>.
     * @param sourceContext {@link OfficeSourceContext}.
     * @return {@link OpenApiArchitect}.
     */
    public static OpenApiArchitect employOpenApiArchitect(OfficeArchitect office,
                                                          WebArchitect web,
                                                          HttpSecurityArchitect security,
                                                          OfficeSourceContext sourceContext) {
        return new OpenApiArchitect() {

            @Override
            public void buildOpenApi(OpenAPI openApi, Set<String> ignorePaths) {

                // Load the security extensions
                List<OpenApiSecurityExtension> securityExtensions = new ArrayList<>();
                for (OpenApiSecurityExtension extension : sourceContext
                ...
                for (OpenApiOperationExtension extension : sourceContext
                ...
                web.addHttpInputExplorer((explore) -> {
                    ...
                    // Ignore paths
                    if (ignorePaths.contains(explore.getRoutePath())) {
                        return;
                    }

                    // Lazy create the path
                    String applicationPath = explore.getApplicationPath();
                    PathItem path = paths.computeIfAbsent(applicationPath, (key) -> new PathItem());
                    ...
                    recursiveLoadManagedFunction(managedFunction, operationBuilder, new HashSet<>());
                });
                ...
                office.addOfficeEscalationExplorer((explore) -> {
                    ...
                    Class<?> escalationClass = sourceContext.loadClass(escalationType);
                    ...
                    for (OperationBuilder operationBuilder : operationBuilders) {
                        ...
                        if ((RuntimeException.class.isAssignableFrom(escalationClass))
                            ...
                            INCLUDED:
                            for (Class<?> unhandledEscalationType : operationBuilder.defaultOperationBuilder
     ...
     * @param operationBuilder {@link OperationBuilder} instances.
     * @param visited          Names of the visited {@link ExecutionManagedFunction}
     *                         instances.
     * @throws Exception If fails to load graph.
     */
    private static void recursiveLoadManagedFunction(ExecutionManagedFunction managedFunction,
        ...
        for (ManagedFunctionFlowType<?> flowType : type.getFlowTypes()) {
            ...
            recursiveLoadManagedFunction(flowManagedFunction, operationBuilder, visited);
        }
        ExecutionManagedFunction nextManagedFunction = managedFunction.getNextManagedFunction();
        if (nextManagedFunction != null) {
            recursiveLoadManagedFunction(nextManagedFunction, operationBuilder, visited);
        }
        for (ManagedFunctionEscalationType escalationType : type.getEscalationTypes()) {
            ...
            if (escalateManagedFunction != null) {
                recursiveLoadManagedFunction(escalateManagedFunction, operationBuilder, visited);
            }
        }
    }
    ...
    private static class OpenApiOperationContextImpl implements OpenApiOperationFunctionContext {
        ...
        public Components getComponents() {
            return OpenApiEmployer.getComponents(this.openApi);
        }
