...
public class HttpSecuritySourceListingWizardPage extends WizardPage {
	...
	/**
	 * {@link SecurityInstance} to refactor or <code>null</code> if creating new
	 ...
	 *            {@link SecurityInstance} to refactor or <code>null</code> if
	 *            creating new.
	 */
	...
	public String getSecurityName() {
		return this.securityNameText.getText();
	}

	/**
	 * Obtains the selected {@link HttpSecuritySourceInstance}.
	 * 
	 * @return Selected {@link HttpSecuritySourceInstance} or <code>null</code> if
	 *         not selected.
	 */
	...
	public void createControl(Composite parent) {
		...
		String initialSecurityName = "";
		long initialAuthenticationTimeout = 3000;
		int initialSectionSourceIndex = -1;
		if (this.securityInstance != null) {
			// Have security instance, so provide initial source details
			initialSecurityName = this.securityInstance.getSecurityName();
			initialAuthenticationTimeout = this.securityInstance.getAuthenticationTimeout();
			String httpSecuritySourceClassName = this.securityInstance.getHttpSecuritySourceClassName();
		...
		// Add means to specify security name timeout
		...
		this.securityNameText = new Text(nameComposite, SWT.BORDER);
		this.securityNameText.setLayoutData(new GridData(SWT.FILL, SWT.BEGINNING, true, false));
		this.securityNameText.setText(String.valueOf(initialSecurityName));
		this.securityNameText.addModifyListener((event) -> this.handleChange());
		...
		this.authenticationTimeoutText.addModifyListener((event) -> this.handleChange());
	...
	protected void handleChange() {
		...
		// Ensure have name
		String securityName = this.securityNameText.getText();
		if (EclipseUtil.isBlank(securityName)) {
