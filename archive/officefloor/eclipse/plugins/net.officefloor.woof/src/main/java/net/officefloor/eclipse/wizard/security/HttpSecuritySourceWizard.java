...
public class HttpSecuritySourceWizard extends Wizard implements HttpSecuritySourceInstanceContext {
	...
	/**
	 * Facade method to obtain the {@link SecurityInstance}.
	 * 
	 * @param editPart
	 *            {@link AbstractOfficeFloorEditPart} to obtain necessary objects to
	 *            run the {@link HttpSecuritySourceWizard}.
	 * @param securityInstance
	 *            {@link SecurityInstance} to base decisions. <code>null</code> if
	 *            creating new {@link SecurityInstance}.
	 * @return {@link SecurityInstance} or <code>null</code> if cancelled.
	 */
	public static SecurityInstance getSecurityInstance(AbstractOfficeFloorEditPart<?, ?, ?> editPart,
		...
		HttpSecuritySourceWizard wizard = new HttpSecuritySourceWizard(project, securityInstance);
		if (WizardUtil.runWizard(wizard, editPart)) {
			// Successful so return the security instance
			return wizard.getSecurityInstance();
		} else {
	 ...
	 */
	private final HttpSecurityAlignWizardPage securityAlignPage;
	 ...
	 */
	private SecurityInstance securityInstance = null;

	/**
	 * Initiate to create a new {@link SecurityInstance}.
	 * 
	 * @param project
	 *            {@link IProject}.
	 */
	private HttpSecuritySourceWizard(IProject project) {
		this(project, null);
	}
	 ...
	 *            {@link IProject}.
	 * @param securityInstance
	 *            {@link SecurityInstance} to be edited, or <code>null</code> to
	 *            create a new {@link SecurityInstance}.
	 */
	private HttpSecuritySourceWizard(IProject project, SecurityInstance securityInstance) {
		...
		// Create the pages
		this.listingPage = new HttpSecuritySourceListingWizardPage(httpSecuritySourceInstanceListing, securityInstance);
		...
		if (securityInstance != null) {
			...
			this.securityAlignPage = new HttpSecurityAlignWizardPage(securityInstance);

			// Load security instance for matching HTTP Security source instance
			String httpSecuritySourceClassName = securityInstance.getHttpSecuritySourceClassName();
			for (HttpSecuritySourceInstance httpSecuritySourceInstance : httpSecuritySourceInstanceListing) {
				if (httpSecuritySourceClassName.equals(httpSecuritySourceInstance.getHttpSecuritySourceClassName())) {
					httpSecuritySourceInstance.loadSecurityInstance(securityInstance);
				}
			}

		} else {
			// Creating new section
			this.securityAlignPage = null;
		}
	}

	/**
	 * Obtains the {@link SecurityInstance}.
	 * 
	 * @return {@link SecurityInstance}.
	 */
	public SecurityInstance getSecurityInstance() {
	...
	public void addPages() {
		...
		// Add refactor pages
		if (this.securityAlignPage != null) {
			this.addPage(this.securityAlignPage);
		}
	}
	...
	public IWizardPage getNextPage(IWizardPage page) {
		...
		if (page == this.currentPropertiesPage) {
			...
			if (this.securityAlignPage != null) {
				// Refactoring security
				this.securityAlignPage.loadHttpSecuritySourceInstance(this.selectedHttpSecuritySourceInstance);
				return this.securityAlignPage;
			}
	...
	public boolean performFinish() {
		...
		// Obtain the details of the security instance
		String securityName = this.listingPage.getSecurityName();
		...
		String[] contentTypes = null; // this.selectedHttpSecuritySourceInstance.getContentTypes();
		...
		if (this.securityAlignPage != null) {
			...
			outputNameMapping = this.securityAlignPage.getOutputNameMapping();
		}
		...
		this.securityInstance = new SecurityInstance(securityName, httpSecuritySourceClassName, authenticationTimeout,
				contentTypes, propertyList, httpSecurityType, outputNameMapping);
